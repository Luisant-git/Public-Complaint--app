...
export function Header({ title, back = false, action = "notification" }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-[68px] items-center justify-between bg-government-600 px-4 text-white shadow-[0_2px_11px_rgba(6,69,34,0.18)]">
      ...
      {action === "notification" ? <button onClick={() => navigate("/notifications")} aria-label="அறிவிப்புகள்" className="relative grid h-9 w-9 place-items-center rounded-full transition hover:bg-white/10"><Bell className="h-5 w-5" /><span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-amber-300 ring-2 ring-government-600" /></button> : action === "menu" ? <Menu className="h-5 w-5" /> : <span className="w-9" />}
    </header>
  );
}