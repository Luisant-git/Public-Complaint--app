import { Bell, ClipboardPlus, Download, MapPinned, WifiOff } from "lucide-react";
...
export function DashboardPage() {
  ...
  return <main className="min-h-dvh pb-24"><Header title="குறை தீர்க்கும் பயணபாடு" /><div className="page-enter px-4 pb-5 pt-6"><div className="flex items-start justify-between"><div><p className="text-sm font-bold text-slate-500">வணக்கம், {userName}</p><h1 className="mt-1 text-xl font-extrabold text-slate-800">உங்கள் குறையை பதிவு செய்யுங்கள்</h1></div>{!isOnline && <span className="flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-700"><WifiOff className="h-3.5 w-3.5" />இணையமில்லை</span>}</div><p className="mt-2 max-w-[310px] text-xs leading-5 text-slate-500">சரியான சேவை வகையை தேர்ந்தெடுத்து உங்கள் குறையை எளிதாக தெரிவிக்கலாம்.</p>
